...
% 03.02.2016 CT check that type is alphabetic, otherwise can't store
%            and cope with empty header data
...
while(~readelementdatafile)
    ...
    if(~isempty(s))
        ...
        while(data(1)==' '); 
            if length(data)==1
                data= '0';  % won't be saved, just to avoid problem
            else
                data=data(2:end); 
            end
        end
    ...
    switch(lower(type))
        ...
        otherwise
            if sum(isstrprop(type,'alpha'))==length(type)
                % only add if character
%                  if isempty(type)
%                      keyboard
%                  else
                      info.(type)=data;
